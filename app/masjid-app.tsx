...
export function MasjidApp() {
  const storageKey = "mbf-active-view";
  ...
  useEffect(() => {
    ...
    try {
      ...
      sessionStorage.setItem("mbf-sidebar-collapsed", String(sidebarCollapsed));
    } catch {
      // Persistence is optional; navigation does not require storage access.
    }
  ...
  const needsAuth = ["keuangan", "jamaah", "master", "pengaturan", "menu"].includes(view);
  if (!viewRestored || (needsAuth && !authReady)) {
    return <div role="status" aria-live="polite" style={{ minHeight: "100dvh", display: "grid", placeItems: "center" }}>Memuat halaman...</div>;
  }
  ...
  return <div className={`${admin ? "shell" : "shell public-site"}${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    <aside className="sidebar" aria-label="Menu utama">
      <button type="button" className="sidebar-toggle" onClick={() => setSidebarCollapsed((value) => !value)} aria-expanded={!sidebarCollapsed} aria-controls="sidebar-navigation" aria-label={sidebarCollapsed ? "Perluas sidebar" : "Ciutkan sidebar"} title={sidebarCollapsed ? "Perluas sidebar" : "Ciutkan sidebar"}>{sidebarCollapsed ? <ChevronRight /> : <ChevronLeft />}</button>
      ...
      <nav id="sidebar-navigation">{nav.map(([id, label, Icon]) => <button key={id} className={view === id ? "active" : ""} onClick={() => setView(id)} aria-label={label} title={sidebarCollapsed ? label : undefined}><Icon /><span className="sidebar-label">{label}</span></button>)}</nav>
      <div className="sidebar-foot">{admin && <button className={view === "pengaturan" ? "active" : ""} onClick={() => setView("pengaturan")} aria-label="Pengaturan" title={sidebarCollapsed ? "Pengaturan" : undefined}><Settings /><span className="sidebar-label">Pengaturan</span></button>}{admin && <button onClick={() => void logout()} aria-label="Keluar" title={sidebarCollapsed ? "Keluar" : undefined}><LogOut /><span className="sidebar-label">Keluar</span></button>}<section><ShieldCheck /><strong>{admin ? (isFullAdmin ? "Mode pengurus aktif" : "Mode staff agenda") : "Data masjid aman"}</strong><small>{admin ? (isFullAdmin ? "Anda dapat mengelola data masjid." : "Anda dapat mengelola agenda kegiatan.") : "Informasi publik dapat dibuka tanpa akun."}</small></section></div>
    </aside>
    <main>
...
function readPrayerLocation(): { lat: number; lon: number } | undefined {
  try {
    const value = JSON.parse(localStorage.getItem("mbf-prayer-location") ?? "null");
    ...
    // Use the default location if browser storage is unavailable or invalid.
  }
  return undefined;
}
...
function usePrayerTimes() {
  ...
  function useGps() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        try {
          localStorage.setItem("mbf-prayer-location", JSON.stringify(coords));
        } catch {
          // GPS still works for this visit when browser storage is unavailable.
        }
        void load(coords).finally(() => setLocating(false));
      },
      () => { setGpsError("Lokasi belum dapat diperbarui. Aktifkan GPS dan izinkan akses lokasi. Lokasi sebelumnya tetap digunakan."); setLocating(false); },
  ...
  useEffect(() => {
    let active = true;
    const coords = readPrayerLocation();
    lastCoords.current = coords;
    fetchPrayerSchedule(coords).then((result) => {
      if (!active) return;
  ...
  return { schedule, dateLabel, location, loading, failed, next, countdown, reload: () => load(lastCoords.current), useGps, locating, gpsError };
}
...
function PrayerPage({ prayer }: { prayer: PrayerState }) {
  ...
  return <div className="stack prayer-page">
    ...
    {failed ? <section className="prayer-error"><Clock3 /><h3>Jadwal salat belum tersedia</h3><p>Aktifkan GPS dan izinkan akses lokasi di browser, lalu tekan tombol &ldquo;Gunakan Lokasi GPS Saya&rdquo; untuk melihat jadwal salat sesuai lokasi Anda.</p><Button variant="outline" onClick={() => void reload()}><RefreshCw />Muat ulang</Button></section> : <div className="prayer-list">{prayerItems.map(({ key, label, icon: Icon }) => <article className={next?.key === key ? "next" : ""} key={key}><span><Icon /></span><div><small>{key === "Sunrise" ? "Matahari terbit" : "Waktu salat"}</small><strong>{label}</strong></div><b>{loading ? "--:--" : schedule?.[key]?.slice(0, 5) ?? "--:--"}</b>{next?.key === key && <em>Berikutnya</em>}</article>)}</div>}
